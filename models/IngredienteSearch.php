<?php

namespace app\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\Ingrediente;

/**
 * IngredienteSearch represents the model behind the search form of `app\models\Ingrediente`.
 */
class IngredienteSearch extends Ingrediente
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['ID_INGREDIENTE'], 'integer'],
            [['NOMBRE_INGREDIENTE', 'UNIDMEDIDA_INGREDIENTE', 'TIPO_INGREDIENTE'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Ingrediente::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'ID_INGREDIENTE' => $this->ID_INGREDIENTE,
        ]);

        $query->andFilterWhere(['like', 'NOMBRE_INGREDIENTE', $this->NOMBRE_INGREDIENTE])
            ->andFilterWhere(['like', 'UNIDMEDIDA_INGREDIENTE', $this->UNIDMEDIDA_INGREDIENTE])
            ->andFilterWhere(['like', 'TIPO_INGREDIENTE', $this->TIPO_INGREDIENTE]);

        return $dataProvider;
    }
}
